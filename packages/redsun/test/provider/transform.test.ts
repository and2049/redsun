...
describe("ProviderTransform.options - Kimi adaptive thinking", () => {
  ...
  test("uses summarized adaptive thinking and effort instead of token budgets", () => {
    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".redsun/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.reasoningVariants", () => {
  ...
  test("uses explicit effort metadata for Anthropic-compatible models", () => {
    ...
    ).toEqual({ max: { thinking: { type: "adaptive", display: "summarized" }, effort: "max" } })
  })
