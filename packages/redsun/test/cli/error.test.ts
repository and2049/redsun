...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/redsun.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/redsun.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/redsun.jsonc", dir: ".redsun", suggestion: "opencode" },
        expected:
          'Directory ".redsun" in /tmp/redsun.jsonc is not valid. Rename the directory to "opencode" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/redsun.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/redsun.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
