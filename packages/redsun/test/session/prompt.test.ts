...
function goalHarness(opts: {
  ...
  const layer = Layer.succeed(
    Goal.Service,
    Goal.Service.of({
      set: (_sessionID, condition) => Effect.sync(() => (active = { condition, react: 0 })),
      get: () => Effect.sync(() => active),
      clear: () => Effect.sync(() => (active = undefined)),
      bumpReact: () =>
        Effect.sync(() => {
          if (!active) return 0
          active = { ...active, react: active.react + 1 }
          return active.react
        }),
...
goalGate.it.instance("goal gate judges completed turns and continues until satisfied", () =>
  Effect.gen(function* () {
    ...
    expect(goalGate.evaluations).toHaveLength(2)
    expect(goalGate.evaluations[0]?.some((message) => message.info.id === seeded.assistant.id)).toBe(true)
    expect(
      messages.some(
        (message) =>
          message.info.role === "user" &&
          message.parts.some(
            (part) => part.type === "text" && part.synthetic === true && part.text.includes("more work remains"),
          ),
      ),
    ).toBe(true)
