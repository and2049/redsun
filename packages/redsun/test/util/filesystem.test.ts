...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "redsun.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "redsun.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "redsun.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "redsun.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["redsun.json", "redsun.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "redsun.json"),
        path.join(project, "redsun.jsonc"),
      ])
    })
  })
