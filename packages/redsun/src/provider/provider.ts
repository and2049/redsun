    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>, model?: Model) {
          ...
          // 1. options.region from redsun.json provider config
...
function fromExtensionProvider(id: string, input: Record<string, unknown>): Info | undefined {
  if (!Array.isArray(input.models)) return
  const providerID = ProviderV2.ID.make(id)
  const kind = typeof input.api === "string" ? input.api : "openai-compatible"
  const npm = kind.includes("anthropic")
    ? "@ai-sdk/anthropic"
    : kind.includes("google")
      ? "@ai-sdk/google"
      : kind.includes("openai-compatible")
        ? "@ai-sdk/openai-compatible"
        : "@ai-sdk/openai"
  const models: Record<string, Model> = {}
  for (const value of input.models) {
    if (!isRecord(value) || typeof value.id !== "string") continue
    const modelID = ModelV2.ID.make(value.id)
    const modes = Array.isArray(value.input) ? value.input : ["text"]
    const prices = isRecord(value.cost) ? value.cost : {}
    models[modelID] = {
      id: modelID,
      providerID,
      api: {
        id: typeof value.api === "string" ? value.api : value.id,
        npm,
        url: typeof input.baseUrl === "string" ? input.baseUrl : "",
      },
      ...
      capabilities: {
        ...
        attachment: modes.includes("image"),
        toolcall: true,
        input: { text: modes.includes("text"), audio: false, image: modes.includes("image"), video: false, pdf: false },
        output: { text: true, audio: false, image: false, video: false, pdf: false },
        interleaved: false,
      },
      cost: {
        input: typeof prices.input === "number" ? prices.input : 0,
        output: typeof prices.output === "number" ? prices.output : 0,
        cache: {
          read: typeof prices.cacheRead === "number" ? prices.cacheRead : 0,
          write: typeof prices.cacheWrite === "number" ? prices.cacheWrite : 0,
        },
      },
      limit: {
        context: typeof value.contextWindow === "number" ? value.contextWindow : 128_000,
        output: typeof value.maxTokens === "number" ? value.maxTokens : 8_192,
      },
      options: {},
  ...
  return {
    ...
    source: "custom",
    env: [],
    key: typeof input.apiKey === "string" ? input.apiKey : undefined,
    options: {
      ...(typeof input.baseUrl === "string" ? { baseURL: input.baseUrl } : {}),
      ...(isRecord(input.headers) ? { headers: input.headers } : {}),
    },
    models,
  }
}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>((ctx) =>
      Effect.gen(function* () {
        ...
        const extensionProviders = Array.from(ExtensionRuntime.providersFor(ctx.directory), ([id, input]) =>
          fromExtensionProvider(id, input),
        ...
        for (const item of extensionProviders) {
          database[item.id] = item
          if (isProviderAllowed(item.id)) providers[item.id] = item
        }
