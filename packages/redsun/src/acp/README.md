...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for Redsun.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal Redsun sessions
...
redsun acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via Redsun's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | redsun acp
```
...
ACP sessions map cleanly to Redsun's internal session model:
