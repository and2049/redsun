...
function createClient(directory: string) {
  const client: MCPClient = new Client(
    { name: "redsun", version: InstallationVersion },
    {
      ...CLIENT_OPTIONS,
      listChanged: {
...
export interface Interface {
  ...
  readonly finishAuth: (mcpName: string, authorizationCode: string, iss?: string) => Effect.Effect<Status, NotFoundError>
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("MCP.state")(function* () {
        const cfg = yield* cfgSvc.getExecutable()
    ...
    const finishAuth = Effect.fn("MCP.finishAuth")(function* (mcpName: string, authorizationCode: string, iss?: string) {
    ...
    const getAuthStatus = Effect.fn("MCP.getAuthStatus")(function* (mcpName: string) {
      const runtimeConfig = (yield* InstanceState.has(state))
        ...
        : undefined
      const mcpConfig = runtimeConfig ?? (yield* cfgSvc.getExecutable()).mcp?.[mcpName]
