...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from redsun.json files
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "redsun"),
    ...(await Filesystem.findUp(["redsun.json", "redsun.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "redsun"))
  }
