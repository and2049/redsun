...
function internalPlugins(flags: RuntimeFlags.Info): PluginInstance[] {
  return [
    ...
    XaiAuthPlugin,
    // REDSUN CLAUDE-CODE: delegated-agent provider registration
    ClaudeCodePlugin,
  ]
}
          ...
          const extension = yield* Effect.tryPromise({
            try: async () =>
              ExtensionRuntime.create({
                plugin: input,
                userEntries: await bridge.promise(config.extensionEntries("user")),
                projectEntries: await bridge.promise(config.extensionEntries("project")),
                defaultTrust: await bridge.promise(config.userDefaultProjectTrust()),
              }),
            catch: errorMessage,
          }).pipe(
