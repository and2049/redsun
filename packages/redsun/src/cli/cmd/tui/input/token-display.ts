...
export function formatCacheHitRatio(tokens: TokenUsage): string | undefined {
  ...
  const denominator = tokens.input + cacheRead + cacheWrite;
  if (denominator <= 0) return "cache 0%";
  return `cache ${Math.round((cacheRead / denominator) * 100)}%`;
}

export function formatContextStatus(input: {
  tokens: TokenUsage;
  contextLimit?: number;
  cacheTokens?: TokenUsage;
}) {
  ...
  const cache = formatCacheHitRatio(input.cacheTokens ?? tokens);
