...
// CLI entry point for `redsun run`.
//
// Sends a single prompt, streams events to stdout, and exits when the session
// goes idle.
//
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run redsun with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to OPENCODE_SERVER_USERNAME or 'redsun')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const message = resolveRunInput(quoted, piped) ?? ""
