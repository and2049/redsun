  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
        ...
        if (args.url) {
          const wellknown = await fetch(`${args.url}/.well-known/redsun`).then((x) => x.json() as any)
        ...
        if (provider === "other") {
          ...
          prompts.log.warn(
            `This only stores a credential for ${provider} - you will need configure it in redsun.json, check the docs for examples.`,
          )
        }
