...
for (const v3 of [false, true])
  test(`usage uses only Kiro's ${v3 ? "v3" : "legacy"} endpoint and releases its process`, async () => {
    ...
    const result = await readKiroUsage(
      { ...agents[0]!, ...(v3 ? { preset: "kiro" } : {}) },
      "/project",
      new AbortController().signal,
      spawn,
    )
    expect(methods).toEqual(
      v3 ? ["initialize", "_kiro/account/getUsage"] : ["initialize", "session/new", "_kiro.dev/commands/execute"],
    )
