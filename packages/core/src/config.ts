...
export class Info extends Schema.Class<Info>("Config.Info")({
  ...
  task_router: Schema.Struct({
    compact: Schema.String.pipe(Schema.optional),
  ...
  defaultProjectTrust: Schema.Literals(["ask", "always", "never"]).pipe(Schema.optional).annotate({
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const discovered = locationIsGlobal
      ...
      : yield* fs
          .up({
            targets: [".redsun", ...names.toReversed()],
    ...
    const directPaths = discovered.filter((item) => path.basename(item) !== ".redsun").toReversed()
    ...
    // global config, project files, then `.redsun` files.
    const configs = [...(supplementary[0] ?? []), ...direct, ...supplementary.slice(1).flat()]
    const trust = yield* fs.readFileStringSafe(path.join(global.config, "trust.json")).pipe(
      Effect.map((text) => {
        if (!text) return undefined
        const data = JSON.parse(text) as Record<string, boolean | null | undefined>
        let current = path.resolve(location.directory).replace(/\\/g, "/")
        while (true) {
    ...
    const globalEntries = supplementary[0] ?? []
    const globalDefault = latest(globalEntries, "defaultProjectTrust")
    const trusted = trust ?? globalDefault === "always"
    const visible = trusted ? configs : globalEntries
