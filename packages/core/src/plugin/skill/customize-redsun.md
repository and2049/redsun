...
top-level keys in `redsun.json` are rejected with `ConfigInvalidError`.
...
```json
{
  ...
  "skills": {
    "paths": [".redsun/skills", "/abs/path/to/skills"],
...
```
.redsun/skills/my-skill/SKILL.md
```
...
```
.redsun/agent/my-reviewer.md      OR     .redsun/agents/my-reviewer.md
```
...
`.redsun/plugin/` or `.redsun/plugins/`.
