...
Import `@opencode/plugin/tui` directly. redsun resolves this import at runtime, so local CLI plugins do not need an
absolute path to a redsun checkout.

## Context

`setup` receives configuration, app metadata, the current location, the redsun client, cached data, theme tokens, the
...
`context.client` is the generated redsun client and can call the connected server, including a remote server.
...
after redsun's health indicators and before the version.
...
Export `./tui` beside the [main plugin](index.md) for automatic loading.
