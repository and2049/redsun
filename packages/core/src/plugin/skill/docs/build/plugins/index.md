...
Plugins under `.redsun/plugins/` are loaded automatically, like the local example above. To load published packages
or plugin directories from other locations, add them to `plugins` in `redsun.json(c)`:
...
export default Plugin.define({
  id: "example",
  setup(ctx) {
    console.log(`loaded in redsun ${ctx.app.version}`)
...
The plugin context is essentially an [redsun server client](../client/index.md).
...
Pass plugin options with the object form in `redsun.json(c)`.
...
Transforms are synchronous edits to redsun's domain state, and each builds on earlier registrations in its domain.
...
After provider transforms and availability are resolved, redsun materializes the active providers' models and replays
...
[`Session.Inbox.User`](https://opencode.ai/v2/docs/api#schema-Session.Inbox.User), [`Session.Inbox.Synthetic`](https://opencode.ai/v2/docs/api#schema-Session.Inbox.Synthetic)
...
`create`; it may differ when a backend requires its own layout. redsun resolves the returned path and uses it for
inventory, startup commands, and the API result. The returned directory must exist.

Strategies choosing another destination handle naming collisions there. redsun still creates the suggested parent
...
const results = await ctx.websearch.query({ query: "redsun plugins", providerID: "internal" })
```
...
Schemas: [`WebSearch.Provider`](https://opencode.ai/v2/docs/api#schema-WebSearch.Provider), [`WebSearch.Result`](https://opencode.ai/v2/docs/api#schema-WebSearch.Result)
...
Multiple plugins can register the same hook. redsun runs them in plugin order,
...
interrupted, redsun does not admit the prompt.
...
redsun does not validate rewritten frames. The driver tracks state from the provider's replies, so a rewrite that
...
Retry hooks can change redsun's initial `decision` by making a terminal failure
...
[RPC contract](rpc.md) for other plugins and clients to import
