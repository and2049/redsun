...
`@opencode/client/effect` is the Effect-native client for the redsun HTTP API. It returns typed Effects and Streams
...
```ts
import { AbsolutePath, Location, redsun } from "@opencode/client/effect"
...
yield *
  client.event.subscribe().pipe(Stream.runForEach((event) => Effect.logInfo("redsun event", { type: event.type })))
```
...
import { redsun } from "@opencode/client/effect"
