...
```ts
import { AbsolutePath, Location, redsun } from "@opencode/sdk/effect"
...
yield *
  opencode.events.subscribe().pipe(
    Stream.runForEach((event) => Effect.logInfo("redsun event", { type: event.type })),
    Effect.forkScoped,
  )
```
...
Customize your redsun instance by registering Effect plugins. Use the embedded
...
```ts
import { redsun } from "@opencode/sdk/effect"
import { Effect } from "effect"
