...
redsun includes a provider and model catalog from [models.dev](https://models.dev). For most providers, connect an
...
Add or override models in a provider's `models` map. The map key is the model ID used by redsun; `modelID` changes the
...
Instead of configuration, `AZURE_RESOURCE_NAME` supplies the resource name to the redsun server. The legacy
`AZURE_COGNITIVE_SERVICES_RESOURCE_NAME` variable also works.

redsun does not query Azure management APIs or discover deployments. If a deployment does not match its catalog
model name, map a redsun model ID to the deployment with `modelID`.
...
Amazon Bedrock uses the AWS default credential chain. A named AWS profile is the simplest durable setup; redsun also
...
Without an explicit region, redsun uses `AWS_REGION`, then `AWS_DEFAULT_REGION`, then `us-east-1`. For a private or
...
Bedrock API keys use `AWS_BEARER_TOKEN_BEDROCK`. Other AWS variables feed SigV4 and are not stored as redsun API-key
accounts.
...
redsun also resolves the project from `GOOGLE_VERTEX_PROJECT`, `GOOGLE_CLOUD_PROJECT`, `GCP_PROJECT`, or
...
```bash
GOOGLE_CLOUD_PROJECT=my-project GOOGLE_VERTEX_LOCATION=europe-west4 redsun --standalone
```
...
For GitHub Enterprise, enter the deployment URL or domain when prompted. redsun uses a Copilot API endpoint returned
...
organization to assign a Copilot seat, then connect again. redsun fetches the account's current model list after login
and whenever the active Copilot account changes.

## Ollama

Start Ollama and pull a model. redsun probes `http://127.0.0.1:11434`, discovers completion models, and adds them to
...
Point redsun at a remote or proxied Ollama server with `settings.baseURL`. Include `/v1`; redsun derives the native
...
models do not appear because redsun only adds models whose Ollama metadata includes the `completion` capability.
...
OpenRouter uses its native runtime and catalog. Provide `OPENROUTER_API_KEY` to the redsun server or connect an
...
| `Cannot initialize provider/model: NAME is required to resolve the provider endpoint` | A `${NAME}` placeholder remains in `baseURL`. Set that variable on the redsun server or replace the template with a complete endpoint.                                         |
