...
OpenCode V1 and redsun can be installed side by side. V1 runs as `opencode`, while V2 installs and runs separately as
`redsun`.
...
Existing supported server config fields, agent definitions, command definitions, skills, and other files in `.redsun/`
...
> **Tip:** If supported V1 functionality does not work in V2, follow the issue-reporting guidance in
> [Troubleshooting](troubleshooting.md) and file a compatibility issue.

> **Warning:** redsun is in beta. Features may break unintentionally, and the server and plugin APIs may continue to change.
...
This section covers both JSON/JSONC configuration and file-based definitions under `.redsun/`.
...
```text
~/.config/redsun/redsun.json(c)
<project>/redsun.json(c)
<project>/.redsun/redsun.json(c)
```
...
### Ask redsun to migrate

The V1 config format remains supported. The native V2 format is optional and makes several settings more explicit and
ergonomic.

The recommended migration path is to ask redsun to update the configuration for you:

```text
Migrate my redsun configuration, including file-based definitions, from the V1 format to the native V2 format.
Preserve its behavior and all unrelated settings.
```

redsun can inspect the complete file, apply the relevant changes below, and avoid rewriting settings that do not need to
...
Nested mixing is intentionally bounded. redsun recognizes mixed V1 and V2 members within `mcp`, `compaction`, and
...
Existing skill files and automatic `.redsun/skills/` discovery do not change. See [Skills](skills.md).
...
You may keep these fields in V1 syntax. redsun normalizes them without warning.
...
V2 discovers skills from both `.redsun/skill/` and `.redsun/skills/`. The preferred layout is:

```text
.redsun/skills/<skill-id>/SKILL.md
```
...
Existing `AGENTS.md` files stay in place. V2 discovers the global `~/.config/redsun/AGENTS.md` and ambient `AGENTS.md`
...
Once the V2 plugin API is finalized, redsun should be able to migrate the majority of V1 plugins while keeping related
local modules and dependencies together. See the current beta [Plugins guide](build/plugins/index.md).

## Server API and clients

redsun has a revised, more ergonomic server API and a new set of clients. Integrations that call the V1 server API
...
during beta, so their contracts may continue to change. See the generated [API reference](https://opencode.ai/v2/docs/api) for the current endpoints,
