...
project. Add one in `redsun.json` or `redsun.jsonc`, then attach its alias in
...
> **Note:** A shorthand string is a local path only when it starts with `.`, `/`, or `~`. Use `./docs`, not `docs`; a bare `docs`
...
Without `branch`, redsun checks out and refreshes the remote's default branch.
...
redsun normalizes each remote and stores one checkout per remote and branch
under its global data directory. Without an explicit branch, the path uses this
shape:

```text
redsun/repos/<host>/<repository-path>
```
...
```text
~/.local/share/redsun/repos/github.com/Effect-TS/effect@main
```
