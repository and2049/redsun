...
References give redsun named access to directories outside the current
project. Use them for documentation, shared libraries, examples, or source from
another repository.

Configure references by alias in `redsun.json` or `redsun.jsonc`:
...
> **Note:** A shorthand string is treated as a local path only when it starts with `.`, `/`, or `~`. Use `./docs`, not `docs`; a
...
Without `branch`, redsun checks out and refreshes the remote's default
...
redsun normalizes a remote and stores one checkout per remote and branch
under its global data directory. Without an explicit branch, the checkout is
stored at `redsun/repos/<host>/<repository-path>`. On a typical Linux
installation, for example, `Effect-TS/effect` is stored at:

```text
~/.local/share/redsun/repos/github.com/Effect-TS/effect
```
...
new user prompt is admitted in their Location, redsun checks them in the
...
> **Warning:** Cached checkouts are shared and can update while an agent is using them. Avoid editing cached checkouts because a
> refresh resets them.
