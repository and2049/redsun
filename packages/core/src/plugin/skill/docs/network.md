...
Set the standard proxy variables and exclude loopback addresses. redsun uses local HTTP connections between the CLI and
...
```bash
redsun service set env HTTP_PROXY http://proxy.example.com:8080
redsun service set env HTTPS_PROXY http://proxy.example.com:8080
redsun service set env NO_PROXY localhost,127.0.0.1,::1
...
```bash
redsun service unset env HTTPS_PROXY
```
...
shown by `redsun service get env`, so do not share that output.

```bash
redsun service set env HTTPS_PROXY 'http://user:p%40ssword@proxy.example.com:8080'
```
...
```bash
redsun service set env NODE_EXTRA_CA_CERTS /etc/company/ca.pem
redsun service start
```

## Foreground

`--standalone` and `serve` use the environment of their own process. `redsun service set env` does not configure these
processes.
