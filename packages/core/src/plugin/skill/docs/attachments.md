...
> **Warning:** Use a model that supports image input before attaching an image. redsun passes supported image media to the selected
...
HTTP and HTTPS attachment URLs are not supported. redsun materializes each
...
Configure image normalization in `redsun.json` or `redsun.jsonc`:
...
> **Note:** These settings apply to supported image media attached directly to prompts and images produced by the built-in `read`
> tool. If the image resizer is unavailable, redsun passes the original image through unchanged.
...
When `auto_resize` is `true`, redsun preserves the aspect ratio, scales the
...
modifying the image. An image that cannot be decoded also fails. If the image resizer cannot be loaded, redsun uses the
...
- Image attachments use provider-native image input. Provider errors can still
  occur when redsun's limits pass but the selected model's limits do not.
