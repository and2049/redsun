...
[API reference](https://opencode.ai/v2/docs/api) for programmatic prompt submission.
...
redsun reads each attachment before admitting the prompt. It rejects invalid
...
With `auto_resize: true`, redsun preserves the aspect ratio and scales down
...
If the image resizer is unavailable, redsun passes the original image through
