...
metadata into a reusable assistant profile. redsun includes agents for common
...
| **Plan** (`plan`)       | `primary`  | Planning agent. File edits are denied except for redsun plan files. Shell commands are not generally denied.                                               |
...
redsun also has hidden `compaction`, `title`, and `summary` system agents.
...
be hidden. If it is unavailable, redsun falls back to `build`, then to the
...
redsun discovers project `.redsun` directories from the current directory
up to the project root. The path below `agents/` becomes the agent ID, so
`.redsun/agents/team/reviewer.md` defines `team/reviewer`.
...
Use the `agents` field in any [redsun configuration file](config.md):
...
subagents because redsun includes it in the subagent catalog shown to the
model.
...
Sets the agent's system prompt. A non-empty value replaces redsun's
...
> **Tip:** Put broad wildcard rules first and exceptions afterward. For example, deny all subagents first, then allow `explore`.
...
redsun removes tools and asks the model to summarize its work in text. New
...
> **Warning:** The current V2 session runner preserves these overlays on the agent definition but does not yet apply them to model
