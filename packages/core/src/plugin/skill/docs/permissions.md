...
> **Warning:** The V1 object syntax uses different field and action names. Do not use `permission`, `bash`, or `task` in V2
...
Rules from lower-priority configuration files are loaded first. redsun then
...
multiple files. redsun denies the operation if any resource resolves to
...
> **Warning:** `shell` runs with the host user's filesystem, process, and network authority. Its resource is raw text, not a parsed
...
| `plan`                    | Uses the same base, allows questions and exiting plan mode, and denies edits except redsun plan files                                   |
...
redsun also permits its managed tool-output, shell-output, temporary, and
