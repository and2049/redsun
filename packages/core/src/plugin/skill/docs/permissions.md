...
inspection, and always block pushes. Add these ordered rules to `redsun.jsonc`:
...
If no rule matches, redsun uses `ask`.
...
```jsonc
{
  "permissions": [{ "action": "edit", "resource": "packages/docs/*.mdx", "effect": "allow" }],
}
```
...
redsun combines rules in order and uses the last match. Lower-priority
...
> **Warning:** `shell` runs with the host user's filesystem, process, and network authority. Directory inference from command text is
...
| `plan`       | Allows questions; denies edits except files under `~/.redsun/plan`                                                         |
...
redsun also allows external-directory access to its managed tool-output,
