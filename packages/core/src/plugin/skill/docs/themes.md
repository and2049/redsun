...
```json title="~/.config/redsun/themes/ocean.json"
{
  "version": 2,
  "dark": {
    "hue": {
...
| Scope   | Directory                             |
| ------- | ------------------------------------- |
| Global  | `~/.config/redsun/themes/`            |
| Project | `.redsun/themes/` in the project tree |

redsun reads `.json` files only. A theme closer to the current directory replaces a global or parent theme with the
same name.
...
Custom themes inherit unspecified values from the built-in redsun theme. To base one mode on the other, add
