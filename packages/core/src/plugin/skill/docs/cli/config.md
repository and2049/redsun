...
Most settings can also be changed from the redsun TUI. Press `Ctrl+P`, then select **Open settings**.
...
redsun includes this automatically when it creates or migrates your CLI config.
...
In `/diff`, press `d` to change the scope or choose a comparison branch from **Base**. redsun infers the base from local Git history when possible; otherwise it asks you to choose before loading a branch diff.
