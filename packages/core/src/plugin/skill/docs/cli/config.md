...
<img src={`${import.meta.env.BASE_URL}assets/docs/cli/settings.png`} alt="redsun TUI settings dialog" />

## Configuration file

redsun stores CLI settings in one global file:

```text
~/.config/redsun/cli.json
```

When `$XDG_CONFIG_HOME` is set, redsun uses:

```text
$XDG_CONFIG_HOME/redsun/cli.json
```
...
redsun adds `$schema` when it creates or migrates this file. Unknown settings are rejected, and valid edits reload
...
redsun merges the inline settings over the global `cli.json`. Nested objects are merged, while arrays and scalar
...
See [Theme](theme.md) for selecting and creating themes.
...
See [Keybinds](keybinds.md) for accepted key syntax, every command ID, and every default binding.
...
See [Plugins](plugins.md) for package forms, local discovery, wildcards, and disable directives.
