...
Configure your ACP client to launch `redsun acp`. For example, add redsun to Zed's
...
`redsun acp` is an [Agent Client Protocol](https://agentclientprotocol.com) agent. The client starts it as a child
process and exchanges newline-delimited JSON-RPC messages over stdin and stdout using ACP protocol version 1.

The command starts a private redsun server for that ACP process. It does not connect to the shared background service,
...
When the client creates a session, it supplies the working directory. redsun loads that directory's configuration,
...
The directory stored on an existing redsun session is authoritative. Loading, resuming, or forking that session uses
...
session removes it from redsun storage.
...
Model, effort, and mode changes are applied to the redsun session, so later prompts use the new selection.

## Content

Prompts can contain text, images, embedded resources, and file resource links. redsun also advertises available slash
...
Clients that support ACP terminal authentication can offer the redsun login command reported during initialization.
The ACP `authenticate` request does not collect credentials itself.

## Troubleshooting

Running `redsun acp` directly appears to wait because it expects ACP messages on stdin. Start it through an ACP client
for normal use.
