...
redsun receives provider credentials through four integration methods:
...
```bash
ANTHROPIC_API_KEY=sk-ant-... redsun --standalone
```

For the shared background server, add the variable to its managed environment. This stops a running service; the next
redsun command starts it with the new value.

```bash
redsun service set env ANTHROPIC_API_KEY sk-ant-...
redsun auth list
```
...
```bash
redsun auth list
redsun service unset env ANTHROPIC_API_KEY
```
...
The usual release path is `~/.local/share/redsun/redsun-release.db`; `XDG_DATA_HOME`, the release channel, and `OPENCODE_DB`
can change it. Do not edit the database to manage credentials; use `/connect` or the `auth` commands.

V2 imports supported credentials from the legacy `auth.json` in the redsun data directory during its database
