...
redsun discovers `.md` command files in `commands/` directories:

```text
~/.config/redsun/commands/       # Global
.redsun/commands/                # Project
```

Files may be nested; for example, `.redsun/commands/team/review.md` defines
...
Add commands under the `commands` key in any redsun JSON or JSONC
...
redsun appends non-empty arguments to the template after a blank line.
...
redsun runs each interpolation with the configured shell in the active
...
> **Warning:** Shell interpolations run when the command is evaluated, outside the agent's tool permission flow. Only use commands
