...
1. The global file at `$XDG_CONFIG_HOME/redsun/AGENTS.md`, normally
...
For example, when the Location is `packages/web`, redsun can load all three
project files below:
...
root. redsun does not resolve conflicts between their contents, so keep broad
...
> **Note:** Current V2 discovery only recognizes `AGENTS.md`. The `CLAUDE.md` fallback and related precedence described by older
> redsun documentation do not apply.

### Nested instructions

An `AGENTS.md` below the Location is not part of the initial upward scan. When
the read tool successfully reads a file or lists a directory, redsun discovers
...
> **Warning:** V2 currently parses and retains this field but does not resolve its entries into instruction sources. Local files,
...
The selected agent or provider system prompt is sent first. redsun then sends
...
System message. During request assembly, redsun renders the epoch's initial
