...
Add an `AGENTS.md` file to give redsun persistent project guidance. Use it for build commands, architecture notes, code conventions, and verification requirements.
...
Place `AGENTS.md` in the directory where its guidance should apply. redsun loads the global file followed by every `AGENTS.md` from the current workspace directory toward the home directory. For workspaces outside the home directory, it stops at the project root.

```text
~/.config/redsun/AGENTS.md
...
Keep guidance that applies everywhere in the global file. Put repository-wide guidance at the project root and more specific guidance closer to the code it covers. redsun combines the files and does not resolve conflicts between them.
...
> **Note:** redsun V2 recognizes `AGENTS.md` only. It does not use `CLAUDE.md` as a fallback.
...
The selected agent or provider system prompt is sent first. redsun then assembles initial instructions in this order:
...
Before the next model request, redsun detects the change and adds an instruction update before delivering pending input:
