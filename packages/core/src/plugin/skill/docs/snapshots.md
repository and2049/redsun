...
Run `/undo` after a response you want to revise. redsun stages the rollback, restores the affected files, and puts the
...
For each model step, redsun attempts a snapshot immediately before the model call and another when the step reaches a
...
redsun stores snapshots locally in a separate internal Git object database under its data directory. Snapshot capture
...
Snapshot objects can contain the complete contents of tracked and eligible untracked files. Treat the redsun data
...
- redsun rejects rollback operations while the session is running, but external editors and commands can still change
