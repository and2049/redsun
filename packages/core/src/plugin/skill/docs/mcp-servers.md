...
redsun connects to [Model Context Protocol](https://modelcontextprotocol.io/) servers and exposes capabilities such as tools, prompts, resources, and instructions. MCP tools consume model context, so add only the servers you need.
...
```sh
redsun mcp add context7 --global --url https://mcp.context7.com/mcp
```

Remote servers use OAuth by default. If the list shows `needs authentication`, open redsun, run `/mcps`, select the server, and sign in. A connected server is ready for an agent to use:
...
A local server is a command that redsun starts over the MCP stdio transport. Add one with a command after `--`:

```sh
redsun mcp add everything -- npx -y @modelcontextprotocol/server-everything
```
...
| `environment` | No       | String variables added to redsun's inherited process environment.                         |
...
OAuth is enabled for remote servers unless `oauth` is `false`. redsun discovers the authorization server, uses PKCE, refreshes tokens, and attempts dynamic client registration when supported; credentials stay outside project configuration.
...
redsun opens every server with the classic MCP `initialize` handshake by default. Set a server's `protocol` to talk to one built on the 2026-07-28 revision:
...
redsun names a tool `<server>_<tool>`. It replaces characters other than letters, numbers, `_`, and `-` with `_`:
...
| Rule     | Behavior                                                                  |
| -------- | ------------------------------------------------------------------------- |
| Identity | It identifies the invoking redsun session, not the MCP transport session. |
...
Use `/mcps` in redsun to view, connect, disconnect, or authenticate servers. Use the CLI to add servers and manage OAuth credentials:

```sh
redsun mcp add sentry --url https://mcp.sentry.dev/mcp
redsun mcp auth sentry
redsun mcp logout sentry
```
