...
redsun can connect to [Model Context Protocol](https://modelcontextprotocol.io/) servers and make their tools, prompts, and instructions available to agents. MCP tools consume model context, so enable only the servers you need.

## Configure servers

Define each server by a unique name under `mcp.servers` in your [redsun configuration](config.md). V2 does not place server names directly under `mcp`.
...
A local server is a command that redsun starts using the MCP stdio transport.
...
| `environment` | No       | String environment variables added to the inherited redsun process environment.                                     |
...
OAuth support is enabled for remote servers unless `oauth` is `false`. redsun discovers the authorization server, uses PKCE, refreshes tokens, and attempts dynamic client registration when the server supports it. OAuth credentials are stored outside project configuration.
...
| --------------- | ----------------------------------------------------------------------------------------------- |
| `client_id`     | Pre-registered OAuth client ID. If omitted, redsun attempts dynamic client registration.        |
...
When redsun invokes an MCP tool on behalf of a session, it includes the invoking
...
invoking redsun session rather than the MCP transport session, can be absent for
...
redsun interfaces can add servers to project or global configuration, list
