...
Create a skill to give an agent reusable instructions for a specific task. Put a `SKILL.md` file in `.redsun/skills/<skill-id>` and describe when to use it in `description`.

```markdown title=".redsun/skills/git-release/SKILL.md"
---
name: Git Release
...
redsun advertises this skill when it is relevant. The agent can then load its instructions with the `skill` tool instead of adding every skill to every prompt.
...
redsun automatically searches these locations:

| Scope                 | Sources                                |
| --------------------- | -------------------------------------- |
| Global                | `~/.config/redsun/skills`              |
| Global compatibility  | `~/.claude/skills`, `~/.agents/skills` |
| Project               | `.redsun/skills`                       |
| Project compatibility | `.claude/skills`, `.agents/skills`     |

For project sources, redsun searches from the current directory up to the project root and includes matching directories at every level.
...
Add more local directories or HTTP catalogs with the `skills` array in any `redsun.json` or `redsun.jsonc`:
...
| Value                       | Resolution                                                    |
| --------------------------- | ------------------------------------------------------------- |
| Relative path               | From the active redsun working directory, not the config file |
...
For that entry, redsun downloads each file from `<base-url>/git-release/<file>`.

| Rule       | Requirement                                                         |
| ---------- | ------------------------------------------------------------------- |
| Paths      | Must be safe, relative, and same-origin                             |
| Entry file | Include `SKILL.md` or `<name>.md`, such as `git-release.md`         |
| Updates    | Increment `version` when files change so redsun refreshes its cache |
...
Skills are selected by ID. If two sources define `git-release`, the source registered later supplies the skill that redsun loads:

```text
~/.config/redsun/skills/git-release/SKILL.md  ← lower precedence
.redsun/skills/git-release/SKILL.md           ← loaded
```
...
4. `~/.config/redsun/skills`
5. Project `.redsun/skills`, from the project root toward the current directory
...
At each model step, redsun lists permitted skills that have a description and do not set `opencode/autoinvoke` to `false`. The list includes only the ID, name, and description, not the full Markdown body.
...
```text
.redsun/skills/release/SKILL.md
                      └─ ID: release
```
