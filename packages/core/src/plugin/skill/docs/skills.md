...
Skills are Markdown instructions that redsun can advertise to an agent and
...
redsun automatically adds the following source directories:
...
For project sources, redsun searches from the current directory upward to
the project root and includes matching directories at every level.

Within each source directory, redsun discovers:
...
Use the `skills` array in any `redsun.json` or `redsun.jsonc` to add local
...
Relative paths are resolved from the active redsun working directory, not
...
redsun downloads those files from
...
implementation. Increment `version` when files change so redsun refreshes
the cached copy.
...
| `metadata.redsun/autoinvoke` | Set to `false` to omit the skill from model-facing discovery       |
...
At each model step, redsun advertises permitted skills that have a
...
When the model calls the `skill` tool with an exact ID, redsun:
