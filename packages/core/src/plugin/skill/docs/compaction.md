...
model limit, redsun creates a checkpoint and retries the pending work with
...
redsun stores the request before waking the session. The response confirms
...
[API reference](https://opencode.ai/v2/docs/api) for the full operation.
...
Before each model call, redsun estimates the final size of the system prompt,
...
- Without provider usage, redsun estimates text, media, instructions, and
  tools locally.
...
redsun also recognizes provider errors classified as context overflow. If no
...
Add `compaction` to any [redsun configuration file](config.md):
...
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Threshold  | `threshold` is an optional positive integer in provider mode. When omitted, redsun uses the model's usable input ceiling; when larger than that ceiling, it is clamped.                            |
...
are stored as returned; redsun does not fabricate a text summary for either
native route.

On a recognized automatic context overflow, redsun attempts local recovery
...
For local compaction, redsun uses the session's selected agent, model, and
...
On later local compactions, redsun updates the previous summary, carries its
...
each actual provider attempt, redsun checks live instruction sources before it
