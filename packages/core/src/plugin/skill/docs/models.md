...
redsun builds its model catalog from [models.dev](https://models.dev), provider integrations, and your configuration.
...
Set `model` in `redsun.json` or `redsun.jsonc`:
...
These values are provider-specific JSON. redsun applies provider values first, then model values, then the selected
...
redsun recognizes `reasoning`, `reasoning_content`, and `reasoning_text`, and accepts any provider-specific string. It
...
redsun automatically discovers language models from an Ollama server listening on its default address,
...
redsun refreshes the inventory in the background and reads context, vision, and tool-use capabilities from Ollama.
...
redsun automatically discovers language models from an unauthenticated LM Studio server listening on its default
...
redsun refreshes the inventory in the background and reads context, vision, and tool-use capabilities from LM
...
redsun automatically discovers models from a vLLM server listening on its default address, `http://127.0.0.1:8000`.
...
redsun checks vLLM's `/health` endpoint and refreshes `/v1/models` in the background. It uses the reported
...
Use the server's real model name, limits, modalities, and tool support. redsun applies the custom-model capability
...
redsun splits the reference at the first `/`, so model IDs may contain additional slashes. Provider and model IDs are
