...
redsun can format files after its `write`, `edit`, or `patch` tools change
...
Set `formatter` to `true` to enable every built-in formatter. redsun runs a
...
redsun includes these formatter definitions. Most require the named command
...
For example, enabling built-ins lets redsun discover and run a project-local
...
`command` is an argument array, not a shell command string. redsun replaces
...
redsun compares the file's final extension with `extensions`. Matching is
...
When several formatters match, redsun tries them in registered order and
...
redsun logs the failure and tries the next match.
