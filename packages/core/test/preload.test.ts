...
describe("Core test environment", () => {
  test("isolates global home and XDG roots", () => {
    ...
    expect(Global.Path.config).toBe(path.join(home, ".config", "redsun"))
    expect(Global.Path.data).toBe(path.join(home, ".local", "share", "redsun"))
    expect(Global.Path.cache).toBe(path.join(home, ".cache", "redsun"))
