...
describe("SessionRunnerLLM", () => {
  ...
  it.effect("retains only complete serialized messages during compaction", () =>
    Effect.gen(function* () {
      ...
      // The hybrid structured inventory repeats a truncated prefix of long
      // messages, so uniqueness is asserted on the tail marker instead.
      expect(summary.match(/EARLIER_END/g)).toHaveLength(1)
  ...
  it.effect("summarizes an oversized newest message without retaining a fragment", () =>
    Effect.gen(function* () {
      ...
      const summary = userTexts(requests[0])[0]
      const continuation = userTexts(requests[1])[0]
      expect(summary.match(/OVERSIZED_END/g)).toHaveLength(1)
