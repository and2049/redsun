...
describe("PluginSupervisor reload", () => {
    ...
    it.live(`reloads helper-defined RPC methods ${scenario.name}`, () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(async () => {
          ...
          await Bun.write(path.join(directory.path, ".redsun/redsun.json"), JSON.stringify({ plugins: [root] }))
        })
        const watcher = yield* Watcher.Test
    ...
    it.effect(`retains a ${mode} plugin change during initial activation`, () =>
      Effect.gen(function* () {
        ...
        const file = path.join(
          directory.path,
          mode === "discovered" ? ".redsun/plugins/greeter.ts" : "external/greeter/index.ts",
        )
        yield* Effect.promise(async () => {
          ...
          if (mode === "configured") {
            await Bun.write(
              path.join(directory.path, ".redsun/redsun.json"),
  ...
  it.live("keeps the running generation when an updated local plugin fails to import", () =>
    Effect.gen(function* () {
      const directory = yield* tmpdirScoped()
      const file = path.join(directory.path, ".redsun/plugins/greeter.ts")
  ...
  it.effect("serializes the periodic refresh behind an in-flight reload", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(path.join(directory.path, ".redsun/redsun.json"), JSON.stringify({ plugins: ["fixture-pkg"] })),
      )
      const bus = yield* Bus.Service
      const locations = yield* LocationServiceMap.Service
      yield* Effect.gen(function* () {
