...
const root = AbsolutePath.make(path.join(project, ".redsun"))
const sources: ConfigDiscovery.Sources = {
  direct: ["redsun.json", "redsun.jsonc"].map((name) => AbsolutePath.make(path.join(project, name))),
...
describe("ConfigWatch.plan", () => {
  test("groups missing candidates and keeps parent watches when roots appear", () => {
    ...
    expect(Array.from(missing.values())).toEqual([
      { path: project, type: "entries", names: [".agents", ".claude", ".redsun", "redsun.json", "redsun.jsonc"] },
    ])
