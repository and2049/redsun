...
export type EventEntryAppended = {
  type: "entry.appended";
  properties: {
    sessionID: string;
    entryID: string;
...
export type EventToolRegistryChanged = {
...
export type KeybindsConfig = {
  ...
  /**
   * Copy current session transcript
   */
  session_copy?: string;
  /**
...
/**
 * Server configuration for redsun serve and web commands
 */
...
export type AgentConfig = {
  model?: string;
  /**
   * Default model variant when this agent uses its configured model
   */
  variant?: string;
  temperature?: number;
  top_p?: number;
  ...
  maxSteps?: number;
  /**
   * Hide the agent from the TUI agent list
   */
  hidden?: boolean;
  permission?: {
    edit?: "ask" | "allow" | "deny";
...
export type ProviderConfig = {
  ...
  options?: {
    ...
    setCacheKey?: boolean;
    /**
     * Opt in to cache_control provider metadata for OpenAI-compatible providers that support it
     */
    openaiCompatibleCacheControl?: boolean;
    /**
     * Experimental OpenAI Responses API continuation mode. Defaults to false.
     */
    experimentalResponsesContinuation?: false | "api-only";
    /**
...
export type Config = {
  ...
  /**
   * Default behavior for project trust prompts (default: ask)
   */
  ...
  small_model?: string;
  /**
   * Per-task model routing for different task types
   */
  task_router?: {
    /**
    ...
    title?: string;
    /**
     * Model for exploration subagents
     */
    explore?: string;
  };
  /**
  ...
  compaction?: {
    ...
    prune?: boolean;
    /**
     * Compaction strategy: 'hybrid' = algorithmic extraction + LLM synthesis (default), 'algorithmic' = rule-based only (0 LLM tokens), 'llm' = full LLM summarization
     */
    strategy?: "hybrid" | "algorithmic" | "llm";
    /**
     * Number of recent messages to keep verbatim in hybrid mode LLM input (default: 4)
     */
    keepRecent?: number;
    /**
     * Context usage ratio that triggers automatic compaction (default: 0.7)
     */
    triggerThreshold?: number;
    /**
     * Context usage ratio that rearms automatic compaction after a trigger (default: 0.4)
     */
    resetThreshold?: number;
    /**
     * Maximum number of tool result summaries to keep in algorithmic compaction inventory (default: 30)
     */
...
export type TrustRespondErrors = {
  /**
   * Bad request
   */
  400: BadRequestError;
...
export type TrustRespondError = TrustRespondErrors[keyof TrustRespondErrors];

export type TrustRespondResponses = {
  /**
   * Trust decision processed
   */
  200: boolean;
};

export type TrustRespondResponse =
  TrustRespondResponses[keyof TrustRespondResponses];
