...
describe("upgrade command", () => {
  ...
  test("accepts an explicit version and method without detection or a version lookup", async () => {
    const result = await cli(["v0.0.0-beta-target", "--method", "powershell"])
    expect(result.exitCode).toBe(0)
    expect(result.events).toEqual([{ method: "powershell", version: "v0.0.0-beta-target" }])
  ...
  test("accepts the short method flag and an explicit major upgrade", async () => {
    const result = await cli(["2.0.0", "-m", "powershell"])
    expect(result.exitCode).toBe(0)
    expect(result.events).toEqual([{ method: "powershell", version: "2.0.0" }])
  })
