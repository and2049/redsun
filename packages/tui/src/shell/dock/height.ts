...
// Dense prompt chrome: textarea + badge row + hint row.
export const DOCK_PROMPT_ROWS = 3
...
export function dockRows(input: {
  ...
  prompt?: boolean
  // Rows the draft in the prompt needs (1 for a single line). The textarea
  // clamps this to its own max height.
  promptRows?: number
}): number {
  ...
  // The declared prompt rows already include one textarea row.
  const promptRows = DOCK_PROMPT_ROWS + Math.max(0, (input.promptRows ?? 1) - 1)
  ...
  const rows = (() => {
    ...
    if (input.view === "dialog") {
      ...
      return DOCK_BASE_ROWS + extra + select + (input.prompt === false ? 0 : promptRows)
    }
    return (
      DOCK_BASE_ROWS +
      extra +
