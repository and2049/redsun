...
export function turnInput(message: SessionMessageAssistant, messages: SessionMessageInfo[], position?: number) {
  const index = position ?? messages.findIndex((item) => item.id === message.id)
  return messages[inputIndex(messages, index === -1 ? messages.length : index)]
}
...
export function turnDuration(message: SessionMessageAssistant, messages: SessionMessageInfo[], position?: number) {
  ...
  const input = turnInput(message, messages, position)
