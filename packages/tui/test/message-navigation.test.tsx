...
test.each([false, true])(
  "normal-mode j/k marks the navigated message, f pins it, right-click opens message actions (scrollbar %s)",
  async (scrollbar) => {
    ...
    const task = Effect.runPromise(
      run({
        ...
        config: { get: async () => ({ animations: false, session: { scrollbar } }), update: async () => ({}) },
    ...
    try {
      ...
      const first = marked(setup.captureCharFrame())
      const start = number(first)
      expect(start).toBeGreaterThanOrEqual(0)
      expect(first.map((line) => line.indexOf("┃"))).toEqual([scrollbar ? 97 : 99])
      setup.mockInput.pressKey("k")
      ...
      await setup.waitForFrame(
        (frame) => marked(frame).some((line) => line.includes("Final answer from the assistant")),
        { maxPasses: 200 },
      )
      expect(marked(setup.captureCharFrame())).toHaveLength(1)
