...
test.each([60, 160])("exit confirmation replaces routine footer details at %i columns", async (width) => {
  const app = await renderFooter({
    width,
    currentAgent: "Build",
    providers: [provider()],
...
test.each(["queue", "steer"] as const)("direct footer toggles and deletes pending %s prompts", async (delivery) => {
  ...
  try {
    await app.renderOnce()
    expect(app.captureCharFrame()).toContain("ctrl+shift+q 1 pending")
    app.mockInput.pressKey("ARROW_DOWN")
    await app.renderOnce()
    expect(app.captureCharFrame()).not.toContain("Pending prompts")

    app.mockInput.pressKey("q", { ctrl: true, shift: true })
    await app.renderOnce()
    const frame = app.captureCharFrame()
    const list = panelMenu(app.renderer.root)
...
test("direct footer counts queued and steering work while running", async () => {
  ...
  try {
    ...
    expect(frame).toContain("ctrl+\\ stop · ctrl+shift+q 2 pending · ctrl+↓ 1 subagent · ctrl+b background · Build")
  ...
  for (const mono of [false, true]) {
    ...
    try {
      for (const state of [
        ...
        { phase: "running" as const, exit: 0, interrupt: 1, notice: "", key: "ctrl+\\", action: "" },
...
test.each([
  { field: "agent", text: "ctrl+\\ stop · GPT-5 [high]" },
  {
    field: "model",
    text: "ctrl+\\ stop · Build · long-model-long-model-lo… [high] · opencode · ctrl+p menu",
  },
  { field: "provider", text: "ctrl+\\ stop · Build · GPT-5 [high]" },
...
test("an oversized queue shortcut prevents backfilling shorter work and identity hints", async () => {
  ...
  try {
    await app.renderOnce()
    const statusline = footerStatusline(app.renderer.root)
    expect(app.captureCharFrame().split("\n")[statusline.y].trimEnd()).toBe("\u25aa ctrl+\\ stop")
  } finally {
    app.cleanup()
  }
})
...
test("direct footer keeps compact work, model detail, and context ahead of cost and rich labels", async () => {
  ...
  try {
    ...
    expect(app.captureCharFrame().split("\n")[statusline.y].trimEnd()).toBe(
      "\u25aa ctrl+\\ stop · ctrl+↓ 1 sub · ctrl+b bg · Plan · a-model-name-long-enough… · 16% ctx",
    )
  } finally {
    app.cleanup()
  }
...
test("direct footer keeps the stop action before the menu at minimum width", async () => {
  const app = await renderFooter({ state: { phase: "running" }, width: 13 })

  try {
    await app.renderOnce()
    expect(app.captureCharFrame()).toContain("ctrl+\\ stop")
...
test.each([false, true])("working marker stays visible with animations=%s and stops at idle", async (animations) => {
  ...
  try {
    ...
    expect(line).toContain(" ctrl+\\ stop")
...
test("direct footer always offers backgrounding for a foreground subagent", async () => {
  ...
  try {
    ...
    expect(frame).toContain("ctrl+↓ 1 subagent · ctrl+b background · Build · gpt-5 · ctrl+p menu")
...
test("direct footer keeps model, variant, and usage before the menu and rich stop label", async () => {
  ...
  try {
    ...
    const statusline = footerStatusline(app.renderer.root)
    expect(app.captureCharFrame().split("\n")[statusline.y].trimEnd()).toBe(
      "* ctrl+\\ stop - Build - GPT-5.6 SoL [high] - 8.4K (1%) - $0.01",
    )
  } finally {
    app.cleanup()
  }
