...
test("explicit theme refresh reloads custom colors without a palette event", async () => {
  ...
  const app = await setup({ mono: false, theme: { name: "mini-refresh" } })
  app.renderer.getPalette = async () => {
    throw new Error("no OSC response")
  }
  try {
...
test("default fallback follows physical mode changes when palette queries remain unavailable", async () => {
